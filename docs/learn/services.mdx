...
`policy_vault` is optional.
When present, the caller configuring the service must be an admin of both the
service vault and the separate policy vault.
...
filter needs.
Policy-vault services cannot themselves invoke another policy filter.
...
- `X-Agent-Vault-Target-URL`: The exact original destination URL.
- `X-Agent-Vault-Continuation-Proxy`: A proxy URL containing a 30-second,
  single-use capability for the exact method and destination URL.
- `X-Agent-Vault-Policy-Proxy`: A reusable 30-second proxy URL scoped to
  `policy_vault`; omitted when no policy vault is configured.
- `X-Agent-Vault-CA`: The Agent Vault MITM root certificate as base64-encoded
  PEM, for a client that continues an HTTPS request.

To allow a request, the filter sends it to `X-Agent-Vault-Target-URL` through
`X-Agent-Vault-Continuation-Proxy` and returns that response.
...
Treat the continuation and policy proxy URLs as short-lived bearer secrets and
do not log them.
The proxy URLs contain bearer capabilities: filters must not persist or log
them.
...
WebSocket upgrades are not supported on filtered services.

Plain HTTP filter URLs must use a literal loopback address.
