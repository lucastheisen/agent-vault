...
A **filter** is an optional sidecar hop on a matched service. After Agent Vault matches the service and **before** it injects credentials, it reverse-proxies the live request to `filter.url`. The sidecar may return any HTTP response to the client (including 403) or complete the call through Agent Vault: either as the minted filter-agent, or with a one-shot continuation ticket on a **new** request to the same method/host/path (the ticket does not hold the original body).

Filters are an operator implementation detail: they do not appear on `/discover`, cannot be set by [proposals](/learn/proposals), and have no CLI flags (YAML-only, same as substitutions).
...
services:
  - name: github-push
    ...
    filter:
      ...
      vault: policy   # optional; defaults to this service's vault
```

- `url` — http(s) origin of the sidecar (loopback is allowed on this hop).
- `vault` — vault the minted `filter-…` agent is granted **proxy** on. Omit to use the same vault (logic in front of the same credentials).
- Agent Vault mints a visible agent named `filter-<vault>-<hash>` (the `filter-` prefix is reserved) and shares it across services with the same `url` + `vault`.
...
- If the sidecar is down or the filter-agent is revoked, the client request fails closed (502/504). Re-apply the YAML to remint a revoked filter-agent.
