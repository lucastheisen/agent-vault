...
<AccordionGroup>
  <Accordion title="agent-vault server">
    ...
    **Environment variables:**
    ...
    | `AGENT_VAULT_FILTER_PROXY_URL` | Advertised MITM forward-proxy URL for remote service-filter sidecars (`https`, or literal-loopback `http`). When unset, local sidecars use the process's loopback MITM listener. |
...
<AccordionGroup>
  ...
  <Accordion title="agent-vault vault service add">
    ...
    [Request filters](/learn/services#request-filters) are also file-only. Add a `filter:` block (`url`, optional `vault`) on a service. `vault service add` preserves an existing filter when the YAML omits `filter`; `vault service set` and `vault service clear` replace the whole list and will drop filters that are not in the file.
  </Accordion>
