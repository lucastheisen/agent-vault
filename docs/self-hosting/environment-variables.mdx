...
| `AGENT_VAULT_FILTER_PROXY_URL` | Optional | Externally reachable forward-proxy URL supplied to remote policy filters for policy and continuation requests. Remote URLs must use HTTPS; plain HTTP is accepted only for a literal loopback address. Usually points to a TLS terminator in front of the MITM proxy listener. |
