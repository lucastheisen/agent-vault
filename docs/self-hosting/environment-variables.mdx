...
| `AGENT_VAULT_FILTER_PROXY_URL` | Optional | Advertised MITM forward-proxy URL for remote service-filter sidecars (`https`, or literal-loopback `http`). When unset, local sidecars use the process's loopback MITM listener. Distinct from `AGENT_VAULT_ALLOW_PRIVATE_RANGES`. |
