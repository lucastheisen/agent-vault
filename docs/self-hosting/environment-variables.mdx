...
| `AGENT_VAULT_FILTER_PROXY_URL` | Optional | Externally reachable forward-proxy URL a policy-filter sidecar should call back on. Only needed when the sidecar runs on a different host from Agent Vault; when unset, the hop advertises this process's own loopback listener, which only a sidecar on the same host can reach. Must be `https` unless it names a literal loopback IP. See [Services: policy filters](/learn/services#policy-filters). Note that neither `AGENT_VAULT_ALLOW_PRIVATE_RANGES` nor `AGENT_VAULT_NETWORK_ALLOWLIST` governs the filter hop: it uses a dedicated dialer with its own rules. |
